...
public class fileUtil {
	public static void save(String contents, String filename, String path)
			throws IOException {
	...
	public static StringBuilder load(String filename, String path)
		...
		if (fileIn.exists() && fileIn.canRead()) {
			BufferedReader buff = new BufferedReader(new FileReader(fileIn));
	...
	/*
	 * public static String load(String filename, String path) throws
	 * IOException { String strOut="";
	 * 
	 * //creating a variable than can interrogate file object File fileIn = new
	 * File(path + filename); //it exists and is not locked if(fileIn.exists()
	 * && fileIn.canRead()) { BufferedReader buff = new BufferedReader(new
	 * FileReader(fileIn)); //anything terminated by a '\n' String strIn =
	 * buff.readLine();
	 * 
	 * while(strIn != null) { strOut += strIn; strIn = buff.readLine();
	 * //StringBuilder } buff.close(); } else { } return strOut; }
	 */
}