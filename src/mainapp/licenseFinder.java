...
public class licenseFinder {
	...
	public void run() {
		...
		// }without the for loop it just prints all the results, and the time
		// taken
	}
	...
	private static String formatInput(String input) {
		...
		// From all that (\/) junk. into this (/\), which is small and all
		// inclusive (and lazy ;))
		/*
		 * int count = 0; for (int i = 0; i < input.length(); i++) { if
		 * (input.charAt(i) == ' ') { count++; } } switch (count) { case 0: if
		 * (String.valueOf(input.charAt(3)).matches("[A-Z]")) { return
		 * input.substring(0,
		 * 2)+" "+input.substring(2,4)+" "+input.substring(4); } else{ return
		 * input.substring(0,
		 * 2)+" "+input.substring(2,3)+" "+input.substring(3); } case 1:
		 * StringBuilder tempo = new StringBuilder(input); String[] spaceFind =
		 * input.split(" "); if (spaceFind[0].length() == 3) { // find the
		 * values of the next two characters in [1] // and find if theyre both
		 * within the ascii alphabet range if (spaceFind[1].substring(0,
		 * 1).matches("[A-Z][A-Z]")) { tempo.insert(spaceFind[0].length() + 2,
		 * ' '); } else if (String.valueOf(spaceFind[1].charAt(0)).matches(
		 * "[A-Z]")) { tempo.insert(spaceFind[0].length() + 1, ' '); } } else {
		 * tempo.insert(3, ' '); } System.out.println(tempo.toString() +
		 * "ye tic"); return tempo.toString(); case 2: // Making assumption that
		 * the spaces are in the right place, // otherwise the plate would have
		 * been dropped in isCorrectFormat() return
		 * input.replaceAll("(\\s|\\t|-){1,}", " "); default: return null; //
		 * Should never get here, but just incase // someone tries to be a
		 * genius }
		 */
	}
}
